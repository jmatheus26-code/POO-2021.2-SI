...
struct Entity {
    ...
    int x {0}; //atributos determinam o estado da minha entidade
    int y {0};
    int step {0};
    sf::Sprite sprite;
...
int main()
{
    ...
    sf::Texture wolf_tex { loadTexture("lobo1.png") };
    sf::Texture rabbit_tex { loadTexture("coelho.png") };
    sf::Texture grass_tex { loadTexture("mundo.jpg") };
    ...
    //scale = final * original 

    while (window.isOpen()) {
        sf::Event event;
        while (window.pollEvent(event)) {
            if (event.type == sf::Event::Closed || (wolf.x == rabbit.x && wolf.y == rabbit.y)) {
                sf::Texture grass_tex { loadTexture(".jpg") };
                board.draw(window);
                window.close();
